...
default_network_acl_egress = [
  ...
  tf_private-subnets = {
    ...
    production = ["172.20.1.0/24","172.20.2.0/24","172.20.3.0/24"]
  }
  private-subnets = local.tf_private-subnets[local.env]

  tf_public-subnets = {
    staging = ["172.16.4.0/24","172.16.5.0/24","172.16.6.0/24"]
    production = ["172.20.4.0/24","172.20.5.0/24","172.20.6.0/24"]
  }
  public-subnets = local.tf_public-subnets[local.env]

  tf_domain = {
    staging = "staging-test.example.local"
    production = "test.example.local"
  }
